...
export class UserService {
  ...
  async unblockUser(user: User, unblockedUser: User) {
    ...
    const [blockingRelationship, blockedRelationship] = await Promise.all([
      this._relationshipModel
        .findOne({
          user: user._id,
    ...
    await Promise.all([
      blockingRelationship?.set({ blocking: false }).save(),
      blockedRelationship?.set({ blocked: false }).save(),
    ]);
  }
